# apps/chat/models/chat_session.py
...
class ChatSessionStatusEnum(models.TextChoices):
...
class ChatSession(TimeStampModel, ActivableModel, AdminStatusModel, CreatorModel):
    """
    Model for chat sessions between users and lawyers/admin/AI
    """

    client = models.ForeignKey(User, on_delete=models.CASCADE, related_name="client_chat_sessions", verbose_name=_("کاربر"))
    consultant = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="consultant_chat_sessions",
    ...
    status = models.CharField(
        max_length=20, choices=ChatSessionStatusEnum.choices, default=ChatSessionStatusEnum.PENDING, verbose_name=_("وضعیت")
    )
    ...
    start_time = models.DateTimeField(null=True, blank=True, verbose_name=_("زمان شروع"))
    end_time = models.DateTimeField(null=True, blank=True, verbose_name=_("زمان پایان"))

    # Reference to the associated request if any
    chat_request = models.OneToOneField(
        "ChatRequest",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="session",  # Changed from "chat_session" to "session"
        verbose_name=_("درخواست گفتگو"),
    )
