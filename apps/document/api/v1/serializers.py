...
class ImageUploadSerializer(TainoBaseSerializer):
...
class VideoUploadSerializer(TainoBaseSerializer):
    creator = serializers.HiddenField(default=serializers.CurrentUserDefault())
...
# apps/document/api/v1/serializers.py
class DocumentUploadSerializer(TainoBaseSerializer):
    creator = serializers.HiddenField(default=serializers.CurrentUserDefault())
    files = serializers.ListField(child=serializers.FileField(), write_only=True)
    file_type = serializers.ChoiceField(choices=NamedContentTypeEnum.labels, required=True, write_only=True)
