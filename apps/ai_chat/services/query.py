...
from apps.ai_chat.models import AISession, AIMessage, ChatRequest, ChatSubscription, AISessionStatusEnum, ChatTypeEnum
...
class ChatQuery(AbstractBaseQuery):
    ...
    @staticmethod
    def get_active_ai_sessions() -> QuerySet[AISession]:
        """
        Get all active chat sessions
        """
        return AISession.objects.filter(status=AISessionStatusEnum.ACTIVE, is_deleted=False)

    @staticmethod
    def get_user_ai_sessions(
        ...
        return AISession.objects.filter(query).order_by("-created_at")
    ...
    @staticmethod
    def get_chat_messages(ai_session_id: str, limit: int = 50, before_message_id: str = None) -> QuerySet[AIMessage]:
        """
        Get messages for a chat session with pagination
        """
        query = Q(ai_session__pid=ai_session_id, is_deleted=False)

        if before_message_id:
            before_message = AIMessage.objects.get(pid=before_message_id)
            query &= Q(created_at__lt=before_message.created_at)

        return AIMessage.objects.filter(query).order_by("-created_at")[:limit]
    ...
    def get_unread_messages_count(user_id: str) -> int:
        ...
        """
        client_unread = AISession.objects.filter(client__pid=user_id, is_deleted=False).aggregate(
            total=Sum("unread_client_messages")
        )

        consultant_unread = AISession.objects.filter(consultant__pid=user_id, is_deleted=False).aggregate(
    ...
    @staticmethod
    def get_ai_session_by_id(session_id: str) -> AISession:
        """
        Get a chat session by ID
        """
        return AISession.objects.filter(pid=session_id, is_deleted=False).first()
